...
Raphael.fn.g.hbarchart = function (x, y, width, height, values, opts) {
	...
	//a.newton hacking here to make this shit work. This function is the only thing altered in this lib.
        labels = labels || [];
        this.labels = paper.set();
        for (var i = 0; i < len; i++) {
            // for (var j = 0; j < multi; j++) {
                var  label = paper.g.labelise(labels[i], values[i], total);
                var X = isRight ? bars[i].x - barheight / 2 + 3 : x + 5,
                    A = isRight ? "end" : "start",
                    L;
                this.labels.push(L = paper.g.text(X, bars[i].y, label).attr({"text-anchor": A}).insertBefore(covers[0]));
                if (L.getBBox().x < x + 5) {
